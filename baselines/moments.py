"""
Road Surface Classification — MOMENT Embedding + Clustering Pipeline

Model      : AutonLab/MOMENT-1-large (HuggingFace, no fine-tuning)
Embeddings : MOMENTPipeline embedding output  → (N, hidden_dim)

Steps
-----
1. Load windowed CSV  →  Z-normalise  →  stratified 80/20 split (labeled only)
2. Load MOMENT pipeline from HuggingFace
3. Compute per-window magnitude  →  extract MOMENT embeddings  →  StandardScaler  →  PCA
...
def setup_logging(log_dir: Path = Path(".")) -> logging.Logger:
    ...
    log_path = log_dir / f"run_{RUN_TS}_moments_cosine_results.log"
...
CONFIG = {
    ...
    "output_csv"             : Path("moments_cosine_results_unlabeled_predictions.csv"),
    "unlabeled_id"           : 0,
    ...
    # MOMENT model
    "moments_model_id"       : "AutonLab/MOMENT-1-large",
    "moments_batch_size"     : 32,
...
def extract_moments_embeddings(pipeline, X_mag: np.ndarray, device,
                                batch_size: int = 32) -> np.ndarray:
    """
    Extract embeddings from MOMENT pipeline.

    Args:
        pipeline   : MOMENTPipeline (task_name="embedding")
        X_mag      : (N, T) float32 magnitude array
    Returns:
        (N, hidden_dim) float32 numpy array
    """
    all_emb = []
    for i in tqdm(range(0, len(X_mag), batch_size), desc="MOMENT embeddings"):
        batch_np = X_mag[i:i + batch_size]
        # MOMENT expects (B, n_channels, T) — univariate so n_channels=1
        batch_tensor = torch.tensor(
            batch_np, dtype=torch.float32
        ).unsqueeze(1).to(device)  # (B, 1, T)
        input_mask = torch.ones(
            batch_tensor.shape[0], batch_tensor.shape[2], dtype=torch.long
        ).to(device)  # (B, T)
        with torch.no_grad():
            output = pipeline(x_enc=batch_tensor, input_mask=input_mask)
        # output.embeddings shape: (B, hidden_dim) — already pooled
        all_emb.append(output.embeddings.detach().cpu().numpy())
...
def main():
    ...
    # ── [3] Load MOMENT pipeline ───────────────────────────────────────────────
    logger.info(f"\n[3] Load MOMENT pipeline: {CONFIG['moments_model_id']}")
    pipeline = MOMENTPipeline.from_pretrained(
        CONFIG["moments_model_id"],
    ...
    # ── [4] Magnitude → MOMENT embeddings → StandardScaler → PCA (labeled) ───
    logger.info("\n[4] Embeddings + PCA (labeled train / test)")
    bs = CONFIG["moments_batch_size"]
    tr_mag = compute_magnitude(tr_X)   # (N_tr, T)
    te_mag = compute_magnitude(te_X)   # (N_te, T)
    tr_emb_raw = extract_moments_embeddings(pipeline, tr_mag, device, bs)
    te_emb_raw = extract_moments_embeddings(pipeline, te_mag, device, bs)
    ...
    if len(X_u) > 0:
        ...
        xu_emb_raw = extract_moments_embeddings(pipeline, xu_mag, device, bs)
