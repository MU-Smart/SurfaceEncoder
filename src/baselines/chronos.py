"""
Road Surface Classification — Chronos Embedding + Clustering Pipeline

Model      : amazon/chronos-t5-tiny (HuggingFace, no fine-tuning)
Embeddings : ChronosPipeline encoder hidden states mean-pooled over patches  → (N, hidden_dim)

Steps
-----
1. Load windowed CSV  →  Z-normalise  →  stratified 80/20 split (labeled only)
2. Load Chronos pipeline from HuggingFace
3. Compute per-window magnitude  →  extract Chronos embeddings  →  StandardScaler  →  PCA
...
from src.baselines.chronos import ChronosPipeline
...
def setup_logging(log_dir: Path = Path(".")) -> logging.Logger:
    ...
    log_path = log_dir / f"run_{RUN_TS}_chronos_cosine_results.log"
...
CONFIG = {
    ...
    "output_csv"             : Path("chronos_cosine_results_unlabeled_predictions.csv"),
    "unlabeled_id"           : 0,
...
def extract_chronos_embeddings(pipeline, X_mag: np.ndarray, device,
                                batch_size: int = 32) -> np.ndarray:
    """
    Extract embeddings from a ChronosPipeline encoder.

    Args:
        pipeline   : ChronosPipeline (tokenizer + model)
        X_mag      : (N, T) float32 magnitude array
    Returns:
        (N, hidden_dim) float32 numpy array (mean-pooled over encoder patches)
    """
    pipeline.model.eval()
    all_emb = []
    for i in tqdm(range(0, len(X_mag), batch_size), desc="Chronos embeddings"):
        batch_np = X_mag[i:i + batch_size]
        batch_tensor = torch.stack([
            torch.tensor(w, dtype=torch.float32) for w in batch_np
        ])  # (B, T) — kept on CPU for tokenisation
        with torch.no_grad():
        ...
        # embedding: (B, n_patches, hidden_dim) → mean over patches
...
def main():
    ...
    logger.info(f"\n[3] Load Chronos pipeline: {CONFIG['chronos_model_id']}")
    pipeline = ChronosPipeline.from_pretrained(
        CONFIG["chronos_model_id"],
        device_map=device,
        dtype=torch.float32,
    )
    pipeline.model.eval()
    logger.info(f"  Parameters: {sum(p.numel() for p in pipeline.model.parameters()):,}")

    # ── [4] Magnitude → Chronos embeddings → StandardScaler → PCA (labeled) ──
    logger.info("\n[4] Embeddings + PCA (labeled train / test)")
    bs = CONFIG["chronos_batch_size"]
    tr_mag = compute_magnitude(tr_X)   # (N_tr, T)
    te_mag = compute_magnitude(te_X)   # (N_te, T)
    tr_emb_raw = extract_chronos_embeddings(pipeline, tr_mag, device, bs)
    te_emb_raw = extract_chronos_embeddings(pipeline, te_mag, device, bs)
    ...
    if len(X_u) > 0:
        ...
        xu_emb_raw = extract_chronos_embeddings(pipeline, xu_mag, device, bs)
